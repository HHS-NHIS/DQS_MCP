// widget/server.js — serves index.html and proxies /ask to core
...
// No-cache headers (avoid stale UI)
...
app.get('/health', async (_req,res)=>{
  ...
  try { const r = await axios.get(CORE_HEALTH, { timeout:2500 });
        info.core.reachable = true; info.core.engine_version = r?.data?.engine_version ?? null;
  } catch(e){ info.core.reachable = false; info.core.error = e.code || String(e); }
  res.json(info);
});
app.get('/version', async (_req,res)=>{
  try { const r = await axios.get(CORE_HEALTH, { timeout:2500 });
        res.json({ engine_version: r?.data?.engine_version ?? null });
...
app.post('/ask', async (req,res)=>{
  try {
    ...
    const detail = err?.response?.data || err?.code || String(err);
...
app.listen(PORT, '0.0.0.0', ()=> {
  console.log(`[widget] ${PORT} → CORE_URL=${CORE_URL} | static=${STATIC_DIR}`);
});
